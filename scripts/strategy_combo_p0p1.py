#!/usr/bin/env python3
"""多策略P0/P1组合回测 (ZhaBan V3 + 首板真低吸)

策略组合：
  P0 (优先级最高): ZhaBan V3 限价单策略
    - 候选: T-1日触及涨停但未封住 (炸板) + 非ST + 非一字板 + 非北交所
    - 过滤: T-1日 turn>15% AND close_rate>5%, T日 open_rate<5%
    - 买入: T日 hour2 限价 = hour1_close * 0.99, 验证 hour2_low <= P
    - 卖出: SL=2% + Trailing=2%, hold_max=T+3

  P1 (补充): 首板真低吸限价单
    - 候选: T-1日首板涨停(前5日无涨停) + turn>10% + T日open_rate∈[-5,+5]
    - 过滤: T日 hour1_close_rate <= -1%
    - 买入: T日 hour2_open 市价买入
    - 卖出: SL=2% + Trailing=2%, hold_max=T+3

组合逻辑:
  1. 每日先生成 P0/P1 候选列表
  2. 共享 N=3 仓位池, P0 信号优先占位
  3. P0 仓位满后, 剩余仓位分配给 P1
  4. 共享资金池, 100万初始资金, 2020-2025六年不重置

合规铁律:
  - 限价P基于事先(上一hour)数据计算, 非事后
  - T+1: T日买入最早T+1卖出
  - 涨停判定: round(close/preclose, 2) >= 1.10(主板) / 1.20(创业板科创板)
  - 卖出止损/Trailing均用 hourX_low 验证

用法:
  python3 scripts/strategy_combo_p0p1.py
"""
import sqlite3
import sys
import math
...
P1_H1CR_MAX = -1.0        # hour1_close_rate <= -1%
P1_SL_PCT = 2.0
P1_TRAIL_PCT = 2.0
P1_HOLD_MAX = 3
# ==================================
...
def find_p0_candidates(by_code, all_dates, date_idx, target_date):
    ...
    for code, dm in by_code.items():
        ...
        # P0 过滤: turn>15% AND close_rate>5%
...
def check_exit_today(by_code, pos, today, today_idx):
    ...
    for h in (1, 2, 3, 4):
        ...
        # 3. Trailing: 当peak > buy_price时, hourX_low <= peak*(1-trail) 触发
        if peak > buy_price:
...
# ============== 买入验证 ==============
def try_buy_p0(cand):
    """P0: hour2限价 P = h1c*(1-offset), 验证h2_low<=P"""
    h1c = cand['h1_close']
    h2l = cand['h2_low']
...
# ============== 回测主循环 ==============
def run_combo_simulation(by_code, all_dates, date_idx):
    capital = float(INITIAL_CAPITAL)
    positions = []
    trades = []
    daily_equity = []  # 每日权益曲线 (用于回撤)

    target_dates = [d for d in all_dates if START_DATE <= d <= END_DATE]
    print(f'\n[回测] 目标交易日: {len(target_dates)} 天 ({target_dates[0]} ~ {target_dates[-1]})', flush=True)
...
def print_results(trades, daily_equity, final_capital):
    print('\n' + '=' * 90)
    print('============ P0/P1 组合策略回测结果 ============')
    print('=' * 90)
...
def main():
    print('=' * 90)
    print('P0/P1 组合策略回测 (ZhaBan V3 + 首板真低吸)')
    print('=' * 90)
    print(f'P0: ZhaBan V3 (炸板修复) - 优先级最高')
    print(f'    过滤: turn>{P0_TURN_MIN}% AND close_rate>{P0_CR_MIN}% AND open_rate<{P0_OPEN_MAX}%')
    print(f'    买入: h1c*(1-{P0_BUY_OFFSET:.3f}) 限价 hour2')
    print(f'    卖出: SL{P0_SL_PCT}% + Trail{P0_TRAIL_PCT}% + T+{P0_HOLD_MAX}强平')
    print(f'P1: 首板真低吸 (补充)')
    print(f'    过滤: 首板+turn>{P1_PREV_TURN_MIN}% + open∈[{P1_OPEN_RATE_MIN},{P1_OPEN_RATE_MAX}] + h1cr<={P1_H1CR_MAX}%')
    print(f'    买入: hour2_open 市价')
    print(f'    卖出: SL{P1_SL_PCT}% + Trail{P1_TRAIL_PCT}% + T+{P1_HOLD_MAX}强平')
    print(f'N={N_SLOTS} 共享仓位, 100万初始资金, {START_DATE} ~ {END_DATE}')
    ...
    trades, daily_equity, final_capital = run_combo_simulation(by_code, all_dates, date_idx)

    print_results(trades, daily_equity, final_capital)
