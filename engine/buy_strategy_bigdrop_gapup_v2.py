"""大阴高开策略V2 - 创业板<50亿，昨日大跌后今日跳空高开"""
from typing import List, Optional
import pandas as pd
from .buy_module import BuyModule


class BigDropGapUpV2Strategy(BuyModule):
    """
    策略逻辑：
    - 昨日创业板小市值股大跌(≥drop_threshold%)
    - 今日跳空高开2-8%（确认资金关注）
    - hour1开盘买入
    - 持有到T+2 hour4卖出

    研究结论：跌≥10%时 +4.05%/58%胜率，跌≥8%时 +1.80%/48%
    """

    strategy_name = 'bigdrop_gapup'

    def __init__(self, buy_hour=1, drop_threshold=-8.0,
                 gap_min=2.0, gap_max=8.0, max_mcap_yi=50.0,
                 target_hold_days=2):
        self.buy_hour = buy_hour
        self.drop_threshold = drop_threshold  # 昨日跌幅阈值(负数,如-8表示跌≥8%)
        self.gap_min = gap_min    # 高开下限%
        self.gap_max = gap_max    # 高开上限%
    ...
    def get_candidates(self, date: str, day_data: pd.DataFrame,
        ...
        for _, row in df.iterrows():
            ...
            if opr is None or pd.isna(opr):
                continue
            if not (self.gap_min <= opr <= self.gap_max):
                continue
            ...
            candidates.append({
                ...
                '_priority': 2 if row['close_rate'] <= -10 else 5,
